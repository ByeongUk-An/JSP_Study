...
<%
	...
	//폼에서 넘겨받은 id와 passwd를 DB 에서 가져온 id, passwd를 비교해서 같으면 수정 일치하지 않으면 Update하지 않는다.
	...
	try {
		...
		rs = pstmt.executeQuery();
			...
			//stmt.executeQuery(sql) : select 한 결과를 ResultSet객체로 값을 반환
		if(rs.next()) {  // DB에 폼에서 넘긴 id 가 존재하면 ==> 폼에서 넘긴 패스워드와 DB의 password가 일치되는지 확인
			...
			if(id.equals(rId) && passwd.equals(rPassword)) {
				//DB에서 가져온 Password와 폼에서 넘긴 패스워드가 일치 할때 Update 한다.
				// sql 변수 재사용
